...
const guideBlock = `        <section className="promptGuide" aria-label="Prompt recipe guide">
          ...
          <div className="guideGrid">
            <article className="guideCard">
              <span>01</span>
              <strong>이미지 툴</strong>
              <p>Image Prompt를 먼저 붙여넣고, 필요하면 Negative Prompt를 마지막에 추가하세요.</p>
            </article>

            <article className="guideCard">
              <span>02</span>
              <strong>영상 툴</strong>
              <p>Video Prompt와 Camera Note를 함께 쓰면 움직임, 렌즈, 조명이 더 안정적으로 잡힙니다.</p>
            </article>
...
if (!app.includes('className="promptGuide"')) {
  ...
  const wrapped = `        <div className="rightStack">
${result.block}

${guideBlock}
        </div>`;

  app = app.slice(0, result.start) + wrapped + app.slice(result.end);
}

fs.writeFileSync(file, app, 'utf8');
console.log('Prompt Recipe Guide를 Prompt Output Board 아래에 추가했습니다.');
