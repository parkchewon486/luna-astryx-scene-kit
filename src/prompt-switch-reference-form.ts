...
function build(source:string,ratio:string,strength:PromptStrength,keepIdentity:boolean,keepWardrobe:boolean,addSafeguards:boolean):Record<ReferenceTool,ReferenceResult>{
  ...
  return {
    gemini:{prompt:gemini,settings:`input: prompt + 1 reference image\naspect_ratio: ${ratio}\nimage_size: 2K`,note:'상세한 자연어와 단계별 장면 지시로 구성했어요.'},
    ...
    midjourney:{prompt:midjourney,settings:`reference: ${keepIdentity?'Omni Reference (V7)':'Image Prompt'}\nreference images: 1\n${keepIdentity?'omni weight: --ow 100\n':''}aspect ratio: --ar ${ratio}\nparameters: prompt end`,note:'긴 문장을 시각 구절로 압축하고 파라미터를 끝에 배치했어요.'},
    grok:{prompt:grok,settings:`workflow: image edit with 1 source image\naspect ratio: requested in prompt (${ratio})`,note:'소스 이미지와 직접적인 자연어 요청을 한 문단으로 정리했어요.'},
  };
}
...
function decorate(root:HTMLElement){root.dataset.referenceForm='4';const identity=root.querySelector<HTMLElement>('[data-switch-identity] + span');if(identity)identity.textContent='레퍼런스 인물 사용';const wardrobe=root.querySelector<HTMLElement>('[data-switch-wardrobe] + span');if(wardrobe)wardrobe.textContent='레퍼런스 의상 유지';const negative=root.querySelector<HTMLSelectElement>('[data-switch-negative]');if(negative){const normal=negative.querySelector<HTMLOptionElement>('option[value="normal"]'),strong=negative.querySelector<HTMLOptionElement>('option[value="strong"]');if(normal)normal.textContent='원문 제외만';if(strong)strong.textContent='기본 오류 방지 추가';if(root.dataset.referenceNegativeReady!=='4'){negative.value='normal';root.dataset.referenceNegativeReady='4';}}
