...
function toolsFor(contest: Contest) {
  ...
  if (/영상|숏폼|광고/.test(categories)) return 'Flow · Seedance · CapCut';
  if (/앱|웹|테크|바이브코딩/.test(categories)) return 'Cursor · Claude Code';
  if (/이미지|디자인|뷰티/.test(categories)) return 'GPT Image · Photoshop';
  return '공식 안내 확인';
}
...
function scoreDetails(contest: Contest) {
  const details = [
    contest.solo_friendly ? '혼자 참가 가능' : '팀 참가 중심',
    contest.beginner_friendly ? '초보 접근 가능' : '경험자 추천',
    `제작 난이도 ${contest.difficulty || '확인 중'}`,
    `예상 제작 ${contest.estimated_days || '확인 중'}`,
  ];
  return details;
}
    ...
    <section className="contestRadar" aria-labelledby="contest-radar-title">
      ...
      <header className="contestRadarHeader">
        <div>
          ...
          <p>AI 활용 여부, 혼자 참가 가능성, 상금과 마감일을 빠르게 비교해 지금 도전하기 좋은 공모전을 골라보세요.</p>
        </div>
      ...
      <div className="contestControls">
        ...
        <select className="contestSort" value={sort} onChange={(event) => setSort(event.target.value as SortKey)} aria-label="공모전 정렬">
        ...
        {contests.map((contest) => {
          ...
          return (
            <article className="contestCard" key={key}>
              ...
              <div className="contestRecommend"><span>추천 이유</span><p>{contest.why_recommended}</p></div>
              <button className="contestExpand" type="button" onClick={() => setExpanded(isExpanded ? null : key)}>{isExpanded ? '점수 기준 닫기' : '점수 기준 보기'}</button>
      ...
      <footer className="contestRadarFooter">접수 전에는 주최기관 공식 페이지에서 마감일과 제출 규격을 다시 확인해 주세요.</footer>
    </section>
  );
}
